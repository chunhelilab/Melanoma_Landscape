...
params={};
params.N=60; %The number of points in the minimum action path.
params.TMax = 1; %The time range of the minimum action path.
%Larger values can be more accurate, but can also lead to instabilities.
params.Dimension = 17; %The dimension of the system.
params.c = 1e18; %The remeshing parameter, c. Larger values give greater remeshing.
params.MaxIter = 10; %The number of optimization steps to run between remeshing steps.
...
for ig=1:NN(2)
  gt=1;
    ...
    if gt==1
       ...
       temp=temp+1;
        x_ss=g_ss; 
        g_ss=zeros(Num,size(x_ss,2)+1);
        g_ss(1:Num,1:size(x_ss,2))=x_ss;
    end 
    weight(1,find(x_ss(bd,:)==YJ(bd,ig)))= weight(1,find(x_ss(bd,:)==YJ(bd,ig)))+1;
end
...
for i=1:size(stable_point,1)
    [m1]=find(xx(:,4)==stable_point(i,4));
    [m2]=find(xx(:,13)==stable_point(i,13));
    m=intersect(m1,m2);
    if length(m)>=1
    ...
    sig=calculate_sigma(xx(m(1),:),par,signal,Num,d)';  
    for j=1:length(m)
        sigma(m(j),:)=sig;
    end
end
...
Func=@(x)Melanoma(1,x,par,signal);  %Force
dFunc=@(x)calculate_sigma(x,par,signal,17,d); 
%Jacobian
feasi_n=size(n,1);
...
AMAM_Params.TMax = 100; %The time range of the minimum action path. 
%Larger values can be more accurate, but can also lead to instabilities. 
